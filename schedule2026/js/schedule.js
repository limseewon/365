const API = "https://script.google.com/macros/s/AKfycbz5KjFugTBHG0klbIBr0JEMzklFzlfEiUt9FuuwkN0dpqYtPQqFOkrF6beNU5Kvt1o/exec";
...
function loadDiaries() {
  try {
    const raw = localStorage.getItem("our_diaries_2026");
...
function saveDiariesToStorage() {
  try {
    localStorage.setItem("our_diaries_2026", JSON.stringify(diaries));
  } catch (e) {
    const slim = diaries.map(d => ({ ...d, media: [] }));
    localStorage.setItem("our_diaries_2026", JSON.stringify(slim));
